...
public enum Board {
     ...
     * 
     * @param firstRow
     *            Row position of the first token.
     * @param firstColumn
     *            Column position of the first token.
     * @param secondRow
     *            Row position of the second token.
     * @param secondColumn
     *            Column position of the second token.
     * @param connectSix
     *            Reference to game control.
     * @return "OK", if the fields were empty and the token positions were different and valid. "PX wins", if player x
     *         wins the game with his current token placements. "draw", if the game ends with a draw.
     * @throws InvalidInputException
     *             if the fields weren't empty nor the token positions were different and valid.
     */
    public String place(final int firstRow, final int firstColumn, final int secondRow, final int secondColumn,
            final ConnectSix connectSix) throws InvalidInputException {
     ...
     * 
     * @param row
     *            Latest row position of a player token.
     * @param column
     *            Latest column position of a player token.
     * @param winCounter
     *            Counts the number of tokens in a row. That means, by adding the win counter to six, this is a win.
     * @param currentPlayer
     *            Current player.
    ...
    private boolean lineCheck(final int row, final int column, final int winCounter, final Player currentPlayer) {
        ...
        // Win situations like: Vertical down, vertical up, horizontal right, horizontal left, diagonal right down,
     ...
     * @param column
     *            Latest column position of a player token.
     * @param rowChangeFactor
     *            Changing factor of a row: 1 is down, -1 is up.
     * @param columnChangeFactor
     *            Changing factor of a column: 1 is right, -1 is left.
     * @param winCounter
     *            Counts the number of tokens in a row. That means, by adding the win counter to six, this is a win.
     * @param currentPlayer
     *            Current player.
     * @return True, if there has been six partnered player tokens in a row. False, if not.
     */
     ...
     * 
     * @param newRow
     *            A possible partnered player token with row position modified by row change factor.
     * @param newColumn
     *            A possible partnered player token with column position modified by column change factor.
     * @param rowChangeFactor
     *            Changing factor of a row: 1 is down, -1 is up.
     * @param columnChangeFactor
     *            Changing factor of a column: 1 is right, -1 is left.
     ...
     * @param currentPlayer
     *            Current player.
     * @return True, if the game mode is torus and a partnered six in a row player token has been found. False, if not.
     */
    ...
    /**
     * Returns a row as a string, if horizontal is true or a column as a string, if horizontal is false and if the given
     * index is valid.
     * 
     * @param index
     *            Row or column index which wants to be printed.
     * @param horizontal
     *            True, if it's a row. False, if it's a column.
     * @return The row or column of the game board as a string.
     * @throws InvalidInputException
     *             if index isn't located on the game board.
     */
     ...
     * @throws InvalidInputException
     *             if the given token position doesn't exist in "standard" game mode.
     */
    public String state(final int row, final int column) throws InvalidInputException {
